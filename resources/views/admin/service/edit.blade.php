...
@section('content')
{!! Form::open(['url' => route('admin.service.edit', $service->id), 'files' => true ]) !!}
...
<div class="form-group {{ $errors->has('summary') ? 'has-error' : '' }}">
    ...
    {{ Form::textArea('summary', old('summary') ?? $service->summary, ['class' => 'form-control']) }}
...
<div class="form-group {{ $errors->has('sort') ? 'has-error' : '' }}">
    ...
    {{ Form::number('sort', old('sort') ?? $service->sort, ['class' => 'form-control']) }}
    @if ($errors->has('sort'))
        <span class="help-block">
...
<div class="form-group {{ $errors->has('is_top') ? 'has-error' : '' }}">
    ...
    <div class="form-control">
        @if (old('is_top') ?? $service->is_top == 1)
            {{ Form::radio('is_top', 1, true) }}
        @else
            {{ Form::radio('is_top', 1) }}
        @endif
        {{ Form::label('&nbsp;是&nbsp;', null, ['class' => 'text-success']) }}
        @if (old('is_top') ?? $service->is_top == 0)
...
<div class="form-group {{ $errors->has('module_id') ? 'has-error' : '' }}">
    ...
    {{ Form::select('module_id', $modules, old('module_id') ?? $service->module_id, ['class' => 'form-control']) }}
...
<script type="text/javascript">
    ...
    ue.on('ready', function() {
        ue.setContent('{!! old('content') ?? $service->content !!}');
    });
    ...
    $('input[name=cover]').fileinput({
        language: 'zh',
        initialPreview: [
            '<img src="{{ $service->cover }}" class="file-preview-image kv-preview-data rotate-1" title="{{ $service->name }}" alt="{{ $service->name }}" style="width:auto;height:auto;max-width:100%;max-height:100%;">'
        ]
    });
</script>
@endsection