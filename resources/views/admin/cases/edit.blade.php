...
@section('content')
{!! Form::open(['url' => route('admin.cases.edit', $case->id), 'files' => true ]) !!}
<div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
    {{ Form::label('案例名', null, ['class' => 'control-label']) }}
    {{ Form::text('name', old('name') ?? $case->name, ['class' => 'form-control']) }}
    @if ($errors->has('name'))
...
<div class="form-group {{ $errors->has('summary') ? 'has-error' : '' }}">
    ...
    {{ Form::textArea('summary', old('summary') ?? $case->summary, ['class' => 'form-control']) }}
...
<div class="form-group {{ $errors->has('sort') ? 'has-error' : '' }}">
    ...
    {{ Form::number('sort', old('sort') ?? $case->sort, ['class' => 'form-control']) }}
    @if ($errors->has('sort'))
        <span class="help-block">
...
<div class="form-group {{ $errors->has('category_id') ? 'has-error' : '' }}">
    ...
    {{ Form::select('category_id', $categories, old('category_id') ?? $case->category_id, ['class' => 'form-control']) }}
...
<script type="text/javascript">
    ...
    $('input[name=cover]').fileinput({
        language: 'zh',
        initialPreview: [
            '<img src="{{ $case->cover }}" class="file-preview-image kv-preview-data rotate-1" title="{{ $case->name }}" alt="{{ $case->name }}" style="width:auto;height:auto;max-width:100%;max-height:100%;">'
        ]
    });
</script>
@endsection