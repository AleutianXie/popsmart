...
{!! Form::open(['url' => route('admin.department.edit', $department->id), 'files' => true ]) !!}
<div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
    {{ Form::label('标签名', null, ['class' => 'control-label']) }}
    {{ Form::text('name', old('name') ?? $department->name, ['class' => 'form-control']) }}
    @if ($errors->has('name'))
...
<div class="form-group {{ $errors->has('comment') ? 'has-error' : '' }}">
    ...
    {{ Form::textArea('comment', old('comment') ?? $department->comment, ['class' => 'form-control']) }}
...
<div class="form-group {{ $errors->has('sort') ? 'has-error' : '' }}">
    ...
    {{ Form::number('sort', old('sort') ?? $department->sort, ['class' => 'form-control']) }}
    @if ($errors->has('sort'))
        <span class="help-block">
...
<div class="form-group {{ $errors->has('is_top') ? 'has-error' : '' }}">
    ...
    <div class="form-control">
        @if (old('is_top') ?? $department->is_top == 1)
            {{ Form::radio('is_top', 1, true) }}
        @else
            {{ Form::radio('is_top', 1) }}
        @endif
        {{ Form::label('&nbsp;是&nbsp;', null, ['class' => 'text-success']) }}
        @if (old('is_top') ?? $department->is_top == 0)
